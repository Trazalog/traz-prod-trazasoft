...
<!-- Modal Reporte de Produccion -->
	<div class="modal modal-fade" id="modal_finalizar" data-backdrop="static" data-keyboard="false">
			<div class="overlay">
			...
			<div class="modal-dialog modal-lm">
					<div id="pnl-1" class="modal-content">
							...
							<div class="modal-footer">
									...
									<button class="btn btn-success" title="Agrear no consumible" onclick="switchPane()">Agregar No Consumibles</i></button>
									<button type="button" class="btn btn-primary" onclick="guardarReporte()">Guardar Reporte</button>
...
<!-- Modal Asociar no consumibles -->
					<div id="pnl-2" class="modal-content hidden">
							...
							<div class="modal-footer">
									...
									<button type="button" class='btn btn-success' onclick='asociarNocos()'>Asociar No Consumibles</button>
							</div>
					</div>
			</div>
	</div>
...
<script>
	// Limpia los modales al cerrar elmodal principal
	...
	// Agregar registro a tabla temporal Reporte
	...
	/// funciones hechas anteriores no se si se usan en algun otro lado
	...
	/// fin funciones hechas anteriores no se si se usan en algun otro lado


	// Agrega no consmibles a tabla temporal en modal asignacion
	...
	// Guarda la info en la tabla y saca modal asignaacion
	...
	// Vuelve a mostrar el modal de reportes
	function resetNoco(){
		// $('#tbl-noco tbody').empty();
			$('#tbl-noco tfoot').show();
			switchPane()
	}
	// Alterna etre modales
	function switchPane(){
	...
	// guarda reporte y asocia noconsumibles a lotes
	function guardarReporte() {

			$tblNoco = $("#tbl-noco tbody tr");
			var tblReportes = $("#tbl-reportes tbody tr td");

			if($("#tbl-reportes tbody tr td").length == 0){
				alert('No puede Guardar un Reporte Vacío');
				return;
			}

			if($("#tbl-noco tbody tr").length == 0){
				alert('Debe asociar No Consumibles al lote');
				return;
			}

			var data = [];
			var batch_id = $('#batch_id_padre').val();
			var batch = {};
			batch.batch_id = batch_id;
			$tblNoco.each(function() {
					var json = {};
					json.batch_id = batch_id;
					json.noco_id = $(this).attr('class');
					data.push(json);
			});
			...
			$.ajax({
					...
					dataType: 'JSON',
					data:{data, batch},
					url: '<?php echo base_url(PRD) ?>general/Noconsumible/asociarConLote',
					success: function(result) {
						if (result.resultado == 'OK') {
								hecho(result.mensaje);
						} else {
								$('#modal_finalizar').modal('hide');
								error();
						}
